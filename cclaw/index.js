    ...
    ws.on('message', (message) => {
        try {
            ...
            if (msg.type === 'auth_success') {
                ...
                const backend = msg.backend || process.env.CCLAW_AI_BACKEND || 'openclaw';
...
function executeOpenClawCommand(command, ws, sessionId, username) {
    ...
    customEnv['OPENCLAW_LOG_LEVEL'] = 'error'; // 抑制 trace/debug/warn 日志，避免内部日志被 PTY 捕获作为 AI 回复内容
    ...
    // 注入系统指令，教导 AI 定时任务消息从网页端返回（不发到微信）
